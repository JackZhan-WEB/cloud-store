...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      showUpdate($index) {
        let member = this.list[$index];
        console.log(member, 'showUpdate');
        this.tempUser.id = member.id;
        this.tempUser.username = member.username;
        this.tempUser.nickname = member.nickname;
        this.tempUser.roles = member.roles;
        this.tempUser.memberId = member.memberId;
        this.tempUser.state = member.state;
        this.tempUser.password = '';
      ...
      removeUser($index) {
        ...
        this.$confirm('确定删除此用户?', '提示', {
          ...
          let member = _vue.list[$index];
          console.log(member, 'member');
          member.state = '4';
          member.roles = null;
          memberService.updateUser(member).then(() => {
            _vue.getList()
          }).catch(() => {
